...
def convert_single_example(ex_index, example, max_seq_length,
  ...
  if ex_index < 5:
    ...
    tf.logging.info("input_value: %f)" % (input_value[0]))
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    features["input_value"] = create_int_feature([int(feature.input_value[0])])
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  name_to_features = {
      ...
      "input_value": tf.FixedLenFeature([1], tf.int64),
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 input_value, use_one_hot_embeddings):
  ...
  with tf.variable_scope("loss"):
    ...
    probabilities = tf.nn.tanh(logits)
    print(probabilities)
    ...
    loss = tf.losses.mean_squared_error(y_prob, probabilities)
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, logits, probabilities) = create_model(
        bert_config, is_training, input_ids, input_mask, segment_ids, input_value, use_one_hot_embeddings)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(input_value, logits, is_real_example):
        ...
        mse = tf.metrics.mean_squared_error(labels=input_value, predictions=predictions, weights=is_real_example)
      ...
      eval_metrics = (metric_fn,
                      [input_value, logits, is_real_example])
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          loss=total_loss,
...
def input_fn_builder(features, seq_length, is_training, drop_remainder):
  ...
  for feature in features:
    ...
    all_input_values.append(feature.input_value)
    ...
    d = tf.data.Dataset.from_tensor_slices({
        ...
        "segment_ids":
            tf.constant(
                ...
                dtype=tf.int32),
        "all_input_values":
            tf.constant(all_input_values, shape=[num_examples, 1], dtype=tf.float32),
    })
    ...
    def create_examples(self, lines, set_type):
        ...
        for (i, line) in tqdm(enumerate(lines)):
            ...
            if set_type == "pred":
                ...
                value = [float(line[0])]
            examples.append(
                InputExample(guid=guid, text_a=text, value=value))
        return examples
